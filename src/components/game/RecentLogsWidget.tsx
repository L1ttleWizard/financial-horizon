...
export function RecentLogsWidget({ log }: { log: LogEntry[] }) {
  ...
  return (
    <div className="bg-white rounded-xl shadow-md p-6 h-full flex flex-col">
      <h2 className="text-2xl font-bold text-gray-700 mb-4">Последние операции</h2>
      <div className="flex-grow">
        {recentLogs.length > 0 ? (
          recentLogs.map(entry => <LogItem key={entry.id} entry={entry} />)
        ) : (
          <p className="text-gray-500 text-center mt-8">История операций пока пуста.</p>
        )}
      </div>
      <Link href="/log" className="mt-4 text-center text-blue-600 font-semibold hover:underline">
        Посмотреть весь лог
      </Link>
    </div>
  );

...
export interface LogEntry { id: string; week: number; type: LogType; description: string; amount: number; }
export interface ActiveDeposit { id: string; bankId: string; bankName: string; amount: number; annualRate: number; term: number; startTurn: number; endTurn: number; }
interface GameOverState { isGameOver: true; reason: GameOverReason; message: string; }
export interface NetWorthHistoryPoint { week: number; netWorth: number; }
...
interface GameState {
    ...
    lastChoiceResult: { outcomeText: string; learningPoint: string; effects: ChoiceEffects; } | null;
...
const initialState: GameState = {
    balance: 500, mood: 70, savings: 0, debt: 0, treeStage: 1, turn: 0,
    activeDeposits: [], availableOffers: [], moodAtZeroTurns: 0,
    gameOverState: { isGameOver: false }, currentEvent: null, log: [],
    netWorthHistory: [{ week: 0, netWorth: 500 }], lastChoiceResult: null,
    isEventModalOpen: false, isResultModalOpen: false, unlockedAchievements: [],
    newlyUnlockedAchievement: null, isGlossaryForced: false, forcedGlossaryTerm: null,
};

// --- ВСПОМОГАТЕЛЬНЫЕ ФУНКЦИИ ---
const addLogEntry = (state: GameState, type: LogType, description: string, amount: number) => {
    if (amount === 0) return;
    state.log.push({ id: `${Date.now()}-${Math.random()}`, week: state.turn + 1, type, description, amount });
};
...
const checkGameOverConditions = (state: GameState) => {
    ...
    if (monthlyInterest > DEBT_SPIRAL_THRESHOLD) {
        state.gameOverState = { isGameOver: true, reason: 'DEBT_SPIRAL', message: 'Ваши долги вышли из-под контроля. Теперь проценты по ним съедают большую часть вашей зарплаты. Вы попали в долговую спираль.' };
        return;
    }
    if (state.mood <= 0) { state.moodAtZeroTurns += 1; } else { state.moodAtZeroTurns = 0; }
    if (state.moodAtZeroTurns >= PAYDAY_CYCLE) {
        state.gameOverState = { isGameOver: true, reason: 'EMOTIONAL_BURNOUT', message: 'В погоне за деньгами вы забыли о себе. Ваше настроение на нуле слишком долго. Без радости и отдыха деньги теряют смысл.' };
        return;
    }
    const netWorth = state.balance + state.savings - state.debt;
    if (netWorth < BANKRUPTCY_NET_WORTH_THRESHOLD && state.mood < BANKRUPTCY_MOOD_THRESHOLD) {
        state.gameOverState = { isGameOver: true, reason: 'BANKRUPTCY', message: 'Вы достигли финансового дна. Ваши долги значительно превышают активы, а моральные силы на исходе. Это банкротство.' };
    }
};
...
const gameSlice = createSlice({
    ...
    reducers: {
        startNextTurn(state) {
            ...
            const shuffledEvents = [...gameEventsPool].sort(() => 0.5 - Math.random());
            state.currentEvent = shuffledEvents[0];
        ...
        confirmGlossaryRead(state) {
            ...
            if (state.balance >= MONTHLY_BILLS) {
                ...
                if (state.balance > 0) { addLogEntry(state, 'expense', 'Частичная оплата счетов', -state.balance); state.balance = 0; }
        ...
        openDeposit(state, action: PayloadAction<{ offer: BankOffer; amount: number; term: number }>) {
            ...
            if (amount > 0 && state.balance >= amount) {
                ...
                state.activeDeposits.push({
                    id: `${Date.now()}`, bankId: offer.id, bankName: offer.bankName, amount,
                    annualRate: offer.annualRate, term, startTurn: state.turn, endTurn: state.turn + term,
                });
...
export const { startNextTurn, confirmGlossaryRead, makeChoice, payDebt, openDeposit, closeResultModal, clearNewAchievement, resetGame } = gameSlice.actions;

...
export default function ProfilePage() {
  ...
  const handleUpdateNickname = async () => {
    ...
    if (newNickname.length < 3) {
      setMessage({ type: 'error', text: 'Никнейм должен содержать не менее 3 символов.' });
      return;
    }
  ...
  const handlePasswordReset = async () => {
    ...
    if (user?.email) {
      try {
        ...
        setMessage({ type: 'error', text: 'Не удалось отправить письмо для сброса пароля.' });
  ...
  const handleDeleteAccount = async () => {
    ...
    try {
      ...
      setMessage({ type: 'error', text: 'Ошибка при удалении аккаунта. Пожалуйста, войдите в систему заново и попробуйте еще раз.' });
      console.error("Account deletion error:", error);
    }
  };

  if (isFetching || loading) {
    return <main className="min-h-screen bg-gray-50 p-4 sm:p-6 flex justify-center items-center"><p>Загрузка...</p></main>;
  }

  if (!user) {
    return <main className="min-h-screen bg-gray-50 p-4 sm:p-6 flex justify-center items-center"><p>Пожалуйста, войдите, чтобы увидеть свой профиль.</p></main>;
  }

  const currentNetWorth = userData?.gameState?.netWorthHistory?.slice(-1)[0]?.netWorth ?? 0;

  return (
    <>
      <main className="min-h-screen p-4 sm:p-6 flex justify-center ">
        <div className="w-full max-w-2xl rounded-xl shadow-lg p-6 sm:p-8 bg-white">
          ...
          <div className="space-y-4">
            ...
            <div className="p-4 bg-gray-50 rounded-lg text-gray-600">
                ...
                />
                <button onClick={handleUpdateNickname} disabled={isUpdatingNickname} className="py-2 px-4 bg-blue-600 hover:bg-blue-700 text-white font-semibold rounded-md shadow-sm disabled:bg-gray-400 whitespace-nowrap">
            ...
            {/* Delete Account */}
            <div className="mt-6">
                <h3 className="text-lg font-semibold text-red-600">Опасная зона</h3>
                <div className="mt-2 flex items-center justify-between p-4 bg-red-50 border border-red-200 rounded-lg">
                <div className='max-w-sm'>
                    <span className="font-medium text-red-700">Удалить аккаунт</span>
                    <p className="text-sm text-gray-500 mt-1">Это действие необратимо. Все ваши данные, включая игровой прогресс, будут удалены навсегда.</p>
                </div>
                <button onClick={() => setIsDeleteModalOpen(true)} className="py-2 px-4 bg-red-600 hover:bg-red-700 text-white font-semibold rounded-md shadow-sm whitespace-nowrap">
                    Удалить
                </button>
                </div>
            </div>
          ...
          {message && (
            <div className={`mt-6 p-4 rounded-md ${message.type === 'success' ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800'}`}>
              {message.text}
            </div>
          )}
        </div>
      ...
      {isDeleteModalOpen && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white rounded-lg shadow-2xl p-8 m-4 max-w-sm w-full">
            <h2 className="text-2xl font-bold text-gray-900 mb-4">Вы уверены?</h2>
            <p className="text-gray-600 mb-6">Это действие приведет к полному удалению вашего аккаунта и всех связанных с ним данных. Отменить это будет невозможно.</p>
            <div className="flex justify-end gap-4">
              <button onClick={() => setIsDeleteModalOpen(false)} className="py-2 px-4 bg-gray-200 hover:bg-gray-300 text-gray-800 font-semibold rounded-md">
                Отмена
              </button>
              <button onClick={handleDeleteAccount} className="py-2 px-4 bg-red-600 hover:bg-red-700 text-white font-semibold rounded-md">
                Да, удалить аккаунт
              </button>
            </div>
          </div>
